...
public class GiftCertificateValidator {
    ...
    private static boolean isValidName(String name) {
        boolean result = ObjectUtils.isNotEmpty(name) && isFoundMatcher(name, NAME_REGEX_VALID);
        log.info("Name '{}' is valid: {}", name, result);
        return result;
    }

    private static boolean isValidDescription(String description) {
        boolean result = ObjectUtils.isNotEmpty(description) && isFoundMatcher(description, DESCRIPTION_REGEX_VALID);
        log.info("Description '{}' is valid: {}", description, result);
        return result;
    }

    private static boolean isValidPrice(BigDecimal price) {
        boolean result = ObjectUtils.isNotEmpty(price) && isFoundMatcher(price.toString(), PRICE_REGEX_VALID);
        log.info("Price '{}' is valid: {}", price, result);
        return result;
    }

    private static boolean isValidDuration(Integer duration) {
        boolean result = ObjectUtils.isNotEmpty(duration) && duration > COUNT_DURATION_MIN && duration < COUNT_DURATION_MAX;
        log.info("Duration '{}' is valid: {}", duration, result);
        return result;
    }

    private static boolean isValidTags(List<Tag> tags) {
        boolean result = ObjectUtils.isNotEmpty(tags);
